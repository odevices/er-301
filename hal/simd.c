...
/* evaluation of 4 sines & cosines at once.
 ...
 almost no extra price so both simd_sin and simd_cos make use of
 simd_sincos..
 */
                             ...
                             const float *__restrict__ phase)
{
  ...
  float32x4_t p = vabsq_f32(vld1q_f32(phase));
  float32x4_t p2 = one - p;
  float32x4_t r0 = vld1q_f32(recent0);
  float32x4_t r1 = vld1q_f32(recent1);
  float32x4_t x = r0 * p + r1 * p2;
  vst1q_f32(out, x);
}
                                ...
                                const float *__restrict__ phase)
{
  ...
  float32x4_t p = vabsq_f32(vld1q_f32(phase));
  float32x4_t p2 = vmulq_f32(p, p);
  float32x4_t r0 = vld1q_f32(recent0);
  float32x4_t r1 = vld1q_f32(recent1);
  float32x4_t r2 = vld1q_f32(recent2);
  float32x4_t x = vmulq_f32(r0, vmulq_f32(vaddq_f32(p, p2), half));
  x = vmlaq_f32(x, r1, vsubq_f32(one, p2));
  x = vmlaq_f32(x, r2, vmulq_f32(vsubq_f32(p2, p), half));
  vst1q_f32(out, x);
}
    ...
    const float *__restrict__ recent2, const float *__restrict__ phase)
{
  ...
  float32x4_t x = vmulq_f32(r0, vmulq_f32(vaddq_f32(p, p2), half));
  x = vmlaq_f32(x, r1, vsubq_f32(one, p2));
  return vmlaq_f32(x, r2, vmulq_f32(vsubq_f32(p2, p), half));
}
    ...
    const float *__restrict__ phase, float32x4_t ep, float gain)
{
  ...
  float32x4_t r1 = vld1q_f32(recent1);
  float32x4_t r2 = vld1q_f32(recent2);
  float32x4_t o = vld1q_f32(out);
  float32x4_t g = vdupq_n_f32(gain);
  float32x4_t x = vmulq_f32(r0, vmulq_f32(vaddq_f32(p, p2), half));
  x = vmlaq_f32(x, r1, vsubq_f32(one, p2));
  x = vmlaq_f32(x, r2, vmulq_f32(vsubq_f32(p2, p), half));
  o = vmlaq_f32(o, x, vmulq_f32(g, ep));
  vst1q_f32(out, o);
}
...
float32x4_t simd_sine_env(const float *envPhase)
{
  ...
  float32x4_t pi = vdupq_n_f32(M_PI);
  float32x4_t ep = vld1q_f32(envPhase);
  // scale to [-pi,pi]
  ep = vsubq_f32(pi2 * ep, pi);
  ep = simd_sin(ep);
  return ep; // Sine
}
...
float32x4_t simd_hanning(const float *envPhase)
{
  ...
  ep = vsubq_f32(pi2 * ep, pi);
  ep = simd_sin(ep);
  return ep * ep; // Hanning
}

// Quadratic interpolation (using 3 points)
void simd_quadratic_interpolate_stereo(float *out, float *recent,
                                       float phase)
{
  float32x2_t r0 = vld1_f32(recent);
  float32x2_t r1 = vld1_f32(recent + 2);
  float32x2_t r2 = vld1_f32(recent + 4);
  float32x2_t p = vabs_f32(vdup_n_f32(phase));
  float32x2_t p2 = vmul_f32(p, p);
  float32x2_t half = vdup_n_f32(0.5f);
  float32x2_t x = vmul_f32(r0, vmul_f32(vadd_f32(p, p2), half));
  x = vmla_f32(x, r1, vsub_f32(vdup_n_f32(1.0f), p2));
  x = vmla_f32(x, r2, vmul_f32(vsub_f32(p2, p), half));
  vst1_f32(out, x);
}
...
float32x4_t simd_boxcar(float32x4_t x)
{
  ...
  uint32x4_t alt = vcaltq_f32(x, one);
  return vbslq_f32(alt, one, zero);
}
...
bool simd_any_greater(float *in, int n, float32x4_t threshold)
{
  ...
  for (int i = 0; i < n; i += 4)
  {
    any = vorrq_u32(any, vcgtq_f32(vld1q_f32(in + i), threshold));
  }

  return vgetq_lane_u32(any, 0) || vgetq_lane_u32(any, 1) || vgetq_lane_u32(any, 2) || vgetq_lane_u32(any, 3);
}
...
bool simd_any_positive(float *in, int n)
{
  ...
  for (int i = 0; i < n; i += 4)
  {
    any = vorrq_u32(any, vcgtq_f32(vld1q_f32(in + i), zero));
  }
...
int simd_first_positive(float *in, int n)
{
  ...
  for (int i = 0; i < n; i += 4)
  {
    uint32x4_t any = vcgtq_f32(vld1q_f32(in + i), zero);
    if (vgetq_lane_u32(any, 0))
    {
      return i;
    }
    else if (vgetq_lane_u32(any, 1))
    {
      return i + 1;
    }
    else if (vgetq_lane_u32(any, 1))
...
int simd_first_positive_4x(float *in, int n)
{
  ...
  {
    uint32x4_t any = vcgtq_f32(vld1q_f32(in + i), zero);
    if (vgetq_lane_u32(any, 0) || vgetq_lane_u32(any, 1) || vgetq_lane_u32(any, 2) || vgetq_lane_u32(any, 3))
    {
      return i;
    }
  }
...
{
  float32x4_t x = vdupq_n_f32(value);
  for (int i = 0; i < n; i += 4)
  {
    vst1q_f32(out + i, x);
  }
