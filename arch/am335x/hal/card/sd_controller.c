...
static void dmaTransferCompletion(uint32_t tcc, EDMA3_RM_TccStatus status,
                                  void *appData)
{
  logAssert(status == EDMA3_RM_XFER_COMPLETE && appData != NULL);
  sd_controller_t *this = NULL;
  this = (sd_controller_t *)appData;
  Event_post(this->hEvents, EVENT_DMA_COMPLETE);
}
...
void sd_controller_init_dma(sd_controller_t *this, EDMA3_DRV_Handle hEdma,
                            uint32_t dmaTx, uint32_t dmaRx)
{
  EDMA3_DRV_Result result = EDMA3_DRV_SOK;

  this->dma.hEdma = hEdma;
  this->dma.chTx = dmaTx;
  this->dma.chRx = dmaRx;
  this->dma.tccTx = EDMA3_DRV_TCC_ANY;
  this->dma.tccRx = EDMA3_DRV_TCC_ANY;

  // request channels
  result = EDMA3_DRV_requestChannel(hEdma, &this->dma.chTx, &this->dma.tccTx,
                                    (EDMA3_RM_EventQueue)DMA_QUEUE_SD,
                                    dmaTransferCompletion, this);
  logAssert(result == EDMA3_DRV_SOK);

  result = EDMA3_DRV_requestChannel(hEdma, &this->dma.chRx, &this->dma.tccRx,
                                    (EDMA3_RM_EventQueue)DMA_QUEUE_SD,
                                    dmaTransferCompletion, this);
  logAssert(result == EDMA3_DRV_SOK);

  // setup rx PaRAM template
  result = EDMA3_DRV_clearErrorBits(this->dma.hEdma, this->dma.chRx);
  logAssert(result == EDMA3_DRV_SOK);

  // src params
  result = EDMA3_DRV_setSrcParams(this->dma.hEdma, this->dma.chRx,
                                  this->baseAddr + CSL_MMCHS_DATA,
                                  EDMA3_DRV_ADDR_MODE_INCR, EDMA3_DRV_W32BIT);
  logAssert(result == EDMA3_DRV_SOK);

  // indexing
  // bidx, cidx
  result = EDMA3_DRV_setSrcIndex(this->dma.hEdma, this->dma.chRx, 0, 0);
  logAssert(result == EDMA3_DRV_SOK);

  result = EDMA3_DRV_setDestIndex(this->dma.hEdma, this->dma.chRx, 4, 512);
  logAssert(result == EDMA3_DRV_SOK);

  // transfer params (acnt, bcnt, ccnt, bcnt_reload)
  result = EDMA3_DRV_setTransferParams(this->dma.hEdma, this->dma.chRx, 4, //acnt
                                       512 / 4,                            //bcnt
                                       0,                                  //ccnt
                                       0,                                  // bcnt_reload
                                       EDMA3_DRV_SYNC_AB);
  ...
  // opt fields
  result = EDMA3_DRV_setOptField(this->dma.hEdma, this->dma.chRx,
  ...
  result = EDMA3_DRV_unlinkChannel(this->dma.hEdma, this->dma.chRx);
  logAssert(result == EDMA3_DRV_SOK);

  EDMA3_DRV_getPaRAM(this->dma.hEdma, this->dma.chRx, &this->rxPaRAM);

  // setup tx PaRAM template
  result = EDMA3_DRV_clearErrorBits(this->dma.hEdma, this->dma.chTx);
  logAssert(result == EDMA3_DRV_SOK);

  // dest params
  result = EDMA3_DRV_setDestParams(this->dma.hEdma, this->dma.chTx,
                                   this->baseAddr + CSL_MMCHS_DATA,
  ...
  // src params
  result = EDMA3_DRV_setSrcParams(this->dma.hEdma, this->dma.chTx,
                                  (uint32_t)0, EDMA3_DRV_ADDR_MODE_INCR,
                                  EDMA3_DRV_W32BIT);
  logAssert(result == EDMA3_DRV_SOK);

  // indexing
  // bidx, cidx
  result = EDMA3_DRV_setDestIndex(this->dma.hEdma, this->dma.chTx, 0, 0);
  logAssert(result == EDMA3_DRV_SOK);

  result = EDMA3_DRV_setSrcIndex(this->dma.hEdma, this->dma.chTx, 4, 512);
  logAssert(result == EDMA3_DRV_SOK);

  // transfer params (acnt, bcnt, ccnt, bcnt_reload)
  result = EDMA3_DRV_setTransferParams(this->dma.hEdma, this->dma.chTx, 4, //acnt
                                       512 / 4,                            //bcnt
                                       0,                                  //ccnt
                                       0,                                  // bcnt_reload
                                       EDMA3_DRV_SYNC_AB);
  ...
  // opt fields
  result = EDMA3_DRV_setOptField(this->dma.hEdma, this->dma.chTx,
                                 EDMA3_DRV_OPT_FIELD_TCINTEN, 1u);
  logAssert(result == EDMA3_DRV_SOK);
  ...
  result = EDMA3_DRV_unlinkChannel(this->dma.hEdma, this->dma.chTx);
  logAssert(result == EDMA3_DRV_SOK);

  EDMA3_DRV_getPaRAM(this->dma.hEdma, this->dma.chTx, &this->txPaRAM);
}
...
#ifdef LOG_EN
      MMCSD_drv_log(
          "MMC:(%p) HS MMC/SD Reset failed", this->baseAddr);
#endif
      ret = SD_FAIL;
    }
    else
...
#ifdef LOG_EN
      MMCSD_drv_log(
          "MMC:(%p) HS MMC/SD Power on failed", this->baseAddr);
#endif
      ret = SD_FAIL;
    }
  }
...
#ifdef LOG_EN
      MMCSD_drv_log(
          "MMC:(%p) HS MMC/SD Bus Frequency set failed", this->baseAddr);
#endif
      ret = SD_FAIL;
    }
...
int32_t sd_controller_dma_xfer(sd_controller_t *this,
                               sd_transaction_t *transaction)
{
  EDMA3_DRV_Result result = EDMA3_DRV_SOK;
  int32_t ret = SD_OK;
  ...
  // DMA is only used for data transfers
  logAssert(transaction->flags & MMCSD_CMDRSP_DATA);

  /* Configure the transfer type */
  cmdObj.enableData = TRUE;
  ...
  cmdObj.cmd.xferType =
      (transaction->flags & MMCSD_CMDRSP_READ) ? HS_MMCSD_XFER_TYPE_RX : HS_MMCSD_XFER_TYPE_TX;
  cmdObj.numBlks = this->xferNumBlocks;

  HSMMCSDIntrClear(this->baseAddr, 0xFFFFFFFF);
  ...
  if (cmdObj.cmd.xferType == HS_MMCSD_XFER_TYPE_RX)
  {
    this->rxPaRAM.destAddr = (unsigned int)transaction->dataBuf;
    this->rxPaRAM.cCnt = transaction->blockCount;
    EDMA3_DRV_setPaRAM(this->dma.hEdma, this->dma.chRx, &this->rxPaRAM);

    /* Enable EDMA for the transfer */
    result = EDMA3_DRV_enableTransfer(this->dma.hEdma, this->dma.chRx,
                                      EDMA3_DRV_TRIG_MODE_EVENT);
    logAssert(result == EDMA3_DRV_SOK);
  }
  else
  {
    this->txPaRAM.srcAddr = (unsigned int)transaction->dataBuf;
    this->txPaRAM.cCnt = transaction->blockCount;
    EDMA3_DRV_setPaRAM(this->dma.hEdma, this->dma.chTx, &this->txPaRAM);

    /* Enable EDMA for the transfer */
    result = EDMA3_DRV_enableTransfer(this->dma.hEdma, this->dma.chTx,
                                      EDMA3_DRV_TRIG_MODE_EVENT);
    logAssert(result == EDMA3_DRV_SOK);
  }

  HSMMCSDIntrStatusEnable(this->baseAddr, INT_EN_MASK & ~(BRR_EN | BWR_EN));
  //	(HS_MMCSD_INTR_MASK_CMDCOMP | HS_MMCSD_INTR_MASK_CMDTIMEOUT |
  //	 HS_MMCSD_INTR_MASK_DATATIMEOUT | HS_MMCSD_INTR_MASK_TRNFCOMP));

  HSMMCSDIntrEnable(this->baseAddr, INT_EN_MASK & ~(BRR_EN | BWR_EN));
  //	(HS_MMCSD_INTR_MASK_CMDCOMP | HS_MMCSD_INTR_MASK_CMDTIMEOUT |
  //	 HS_MMCSD_INTR_MASK_DATATIMEOUT | HS_MMCSD_INTR_MASK_TRNFCOMP));
  ...
  /*
    ...
    Event_pend(this->hEvents, EVENT_CMD_COMPLETE, Event_Id_NONE,
            BIOS_WAIT_FOREVER);
#else
  if (Event_pend(this->hEvents, EVENT_CMD_COMPLETE, Event_Id_NONE, 1000) == 0)
  {
    this->waitTimeout++;
    this->cmdTimeout = 1;
  }
  ...
  {
    /*
         * Wait for the DMA to complete here.
         */
#if 0
        Event_pend(this->hEvents, EVENT_XFER_COMPLETE, Event_Id_NONE,
                BIOS_WAIT_FOREVER);
#else
    if (Event_pend(this->hEvents, EVENT_XFER_COMPLETE, Event_Id_NONE, 1000) == 0)
    {
      this->waitTimeout++;
      this->xferTimeout = 1;
    }
    ...
    if (1 == this->xferComp)
    {
      ...
      this->xferComp = 0;
        ...
        if(SD_OK == ret)
        {
            //Event_pend(this->hEvents,EVENT_DMA_COMPLETE,Event_Id_NONE,BIOS_WAIT_FOREVER);
            while(Event_pend(this->hEvents,EVENT_DMA_COMPLETE,Event_Id_NONE,1000)==0)
            {
                EDMA3_DRV_PaRAMRegs param;
                EDMA3_DRV_Result result;
                result = EDMA3_DRV_getPaRAM(this->dma.hEdma,this->dma.chRx,&param);
                logAssert(result==EDMA3_DRV_SOK);
                Breakpoint();
            }
            result = Edma3_CacheInvalidate((uint32_t)transaction->dataBuf,
                    CACHE_ALIGNED_SIZE(transaction->blockSize * transaction->blockCount));
            logAssert(result==EDMA3_DRV_SOK);
        }
#endif
  }
  ...
  /*
     if(cmdObj.cmd.xferType==HS_MMCSD_XFER_TYPE_RX){
     result = EDMA3_DRV_disableTransfer(this->dma.hEdma, this->dma.chRx, EDMA3_DRV_TRIG_MODE_EVENT);
     logAssert(result==EDMA3_DRV_SOK);
     } else {
     result = EDMA3_DRV_disableTransfer(this->dma.hEdma, this->dma.chTx, EDMA3_DRV_TRIG_MODE_EVENT);
     logAssert(result==EDMA3_DRV_SOK);
     }
     */
...
bool sd_controller_test_card_present(sd_controller_t *this)
{
  ...
  if (sd_controller_xfer(this, &transaction) == SD_OK)
  {
