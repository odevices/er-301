...
local AliasingTriangle = Class {}
AliasingTriangle:include(Unit)

function AliasingTriangle:init(args)
  args.title = "Aliasing Triangle"
  args.mnemonic = "Tr"
  Unit.init(self, args)
end

function AliasingTriangle:onLoadGraph(channelCount)
...
function AliasingTriangle:loadMonoGraph()
  local osc = self:addObject("osc", libcore.TriangleOscillator())
...
function AliasingTriangle:loadStereoGraph()
  self:loadMonoGraph()
...
function AliasingTriangle:onLoadViews(objects, branches)
  local controls = {}
...
function AliasingTriangle:deserialize(t)
