...
local AliasingSaw = Class {}
AliasingSaw:include(Unit)

function AliasingSaw:init(args)
  args.title = "Aliasing Saw"
  args.mnemonic = "Sw"
  Unit.init(self, args)
end

function AliasingSaw:onLoadGraph(channelCount)
...
function AliasingSaw:loadMonoGraph()
  local osc = self:addObject("osc", libcore.SawtoothOscillator())
...
function AliasingSaw:loadStereoGraph()
  self:loadMonoGraph()
...
function AliasingSaw:onLoadViews(objects, branches)
  local controls = {}
...
function AliasingSaw:deserialize(t)
