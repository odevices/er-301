...
local TestOscillator = Class {}
TestOscillator:include(Unit)

function TestOscillator:init(args)
  args.title = "Test Osc"
  args.mnemonic = "Si"
  Unit.init(self, args)
end

function TestOscillator:onLoadGraph(channelCount)
...
function TestOscillator:loadMonoGraph()
  local osc = self:addObject("osc", app.TestOscillator())
...
function TestOscillator:loadStereoGraph()
  self:loadMonoGraph()
...
function TestOscillator:onLoadViews(objects, branches)
  local controls = {}
...
function TestOscillator:deserialize(t)
