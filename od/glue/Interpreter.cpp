...
namespace od
{
  ...
  void Interpreter::dumpStack(const char * label)
  {
    int i;
    ...
    { /* repeat for each level */
      ...
      case LUA_TSTRING: /* strings */
        logRaw("`%s' ", lua_tostring(L, i));
        break;

      case LUA_TBOOLEAN: /* booleans */
        logRaw(lua_toboolean(L, i) ? "true " : "false ");
        break;

      case LUA_TNUMBER: /* numbers */
        logRaw("%g ", lua_tonumber(L, i));
        break;

      default: /* other values */
        logRaw("%s ", lua_typename(L, t));
        break;
      }
    }
    logRaw("]\n"); /* end the listing */
