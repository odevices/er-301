...
namespace od
{
  ...
  void SliceList::setSamplePeriod(float period)
  {
  ...
  float SliceList::getPositionInSeconds(int index)
  {
    Slice *slice = mpSlices->get(index);
    if (slice)
    {
  ...
  void SliceList::draw(FrameBuffer &fb)
  {
    ...
    for (i = mTopIndex, j = 0, y = top - mTextSize / 2; i < n && y >= bottom;
         i++, j++, y -= mTextSize)
    {
      float secs = getPositionInSeconds(i);
