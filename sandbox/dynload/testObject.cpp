...
namespace testObject
{
  ...
  MicroDelay::MicroDelay(const std::string &name, float secs) : od::Object(name)
  {
    addInput(mInput);
    addParameter(mDelay);
    addOutput(mOutput);
...
extern "C"
{
  bool runTestObject()
  {
    logInfo("runTestObject called.");
    testObject::MicroDelay obj("MicroDelay Instance", 3);
    obj.process();
    obj.process();
    logInfo("runTestObject exiting.");
    return true;
  }
}
